...
struct Snake {
    ...
    void reset() {
        body = { {windowWidth / 2.0f, windowHeight / 2.0f} };
...
struct Food {
    ...
    void respawn(const Snake& snake) {
        bool valid = false;
        while (!valid) {
            position.x = static_cast<float>((rand() % (static_cast<int>(windowWidth / gridSize))) * gridSize);
            position.y = static_cast<float>((rand() % (static_cast<int>(windowHeight / gridSize))) * gridSize);
            valid = true;
